        ...
        <DialogContent>
            <form @submit="onSubmit">
                ...
                <div class="my-4 grid gap-1.5">
                    ...
                    >
                        <NumberFieldContent>
                            <NumberFieldInput place class="pl-2 text-left" />
                        </NumberFieldContent>
                    </NumberField>
...
const onSubmit = handleSubmit(async (values) => {
    ...
    const [response, err] = await bankStore.deposit(account, values.amount * 100);

    if (err) {
        isProcessing.value = false;
        return toasty.error(`Failed to deposit funds`);
    }

    console.log('returned data', response);
    if (response?.data.account) {
        emit('deposit', response.data.account);
    }
